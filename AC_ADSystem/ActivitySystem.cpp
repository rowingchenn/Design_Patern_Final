...
AcNode_Discount::AcNode_Discount(int ID, string Cont, vector<int> &CommodityInformationIDList, float DisRate) : AcNode(
        ID, Cont) {
...
bool AcNode_Discount::IsSatisfy(CommodityInformation *BuyCommodityInformation) {
    ...
    reader->setCommodityInformation(BuyCommodityInformation);
    for (vector<int>::iterator iter = SatisfyCommodityInformationIDList.begin();
         iter != SatisfyCommodityInformationIDList.end(); ++iter) {
...
void ActivitySystem::PrintAc_D(int Index) {
    ...
    cout << "活动" << DiscountActivityList[Index]->AcID << ": " << DiscountActivityList[Index]->Content << endl;
}
